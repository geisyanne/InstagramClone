...
class FireProfileDataSource : ProfileDataSource {
            ...
            .addOnSuccessListener { res ->
                ...
                when (user) {
                    null -> {
                        ...
                        if (user.uuid == FirebaseAuth.getInstance().uid) { // USER LOGADO
                            callback.onSuccess(Pair(user, null))
                        } else {  // CHEGOU NA TELA ATRAVÉS DA TELA DE PESQUISA, SEARCH
                            FirebaseFirestore.getInstance()
    ...
    override fun followUser(
        ...
        FirebaseFirestore.getInstance()
            ...
            .addOnFailureListener { exception ->
                ...
                // CREATE
                if (err?.code == FirebaseFirestoreException.Code.NOT_FOUND) {  // SE FALHA FOR COLEÇÃO NÃO EXISTIR
                    FirebaseFirestore.getInstance()
                        .collection("/followers")
                        .document(userUUID)
                        .set(
    ...
    private fun updateFeed(userUUID: String, isFollow: Boolean) {
        if (!isFollow) {  // SE NÃO SEGUIR, REMOVER DO FEED
            FirebaseFirestore.getInstance()
                .collection("/feeds")
